...
IMPORTANT: Respond in ${langName}. The user is ${age} years old. Adjust your language complexity accordingly.`;
  ...
  async sendMessage(
    ...
    // 모든 모델 실패 시 친절한 에러 메시지
    console.error('All Gemini models failed:', lastError);
    
    const errorMessage = lastError?.message || 'Unknown error';
    if (errorMessage.includes('503') || errorMessage.includes('Service Unavailable')) {
      throw new Error('서비스가 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해주세요. (Service temporarily unavailable. Please try again later.)');
    }
    
    throw new Error(`AI 서비스 오류: ${errorMessage}`);
  }
